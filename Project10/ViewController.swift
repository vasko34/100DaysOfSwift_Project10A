...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        performSelector(inBackground: #selector(loadPeople), with: nil)
    ...
    func savePeople() {
        ...
        if let savedPeople = try? jsonEncoder.encode(people) {
            defaults.setValue(savedPeople, forKey: "people")
        }
    }
    ...
    @objc func loadPeople() {
        ...
        if let peopleToLoad = defaults.object(forKey: "people") as? Data {
            do {
                peopleSecure = try jsonDecoder.decode([Person].self, from: peopleToLoad)
            } catch {
    ...
    @objc func unlockApp() {
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { [weak self] success, error in
                DispatchQueue.main.async {
                    if success {
                        ...
                        if let peopleSecure = self?.peopleSecure {
                            self?.people = peopleSecure
                            self?.collectionView.reloadData()
                        }
                    } else {
                        let ac = UIAlertController(title: "Failed to unlock", message: "Please try again.", preferredStyle: .alert)
            ...
            let ac = UIAlertController(title: "Error", message: "Biometric authentication is disabled or unavailable.", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
            present(ac, animated: true)
        }
    }
